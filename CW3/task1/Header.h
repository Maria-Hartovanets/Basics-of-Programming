...
void writeStructToConsole(Book book)   // виведення структури на консоль
{
	cout.width(16);
...
void writeMasToConsole(Book* mas, int n) // виводимо на консоль масив структур
{
	for (int i = 0; i < n; i++)  // в циклі виводимо кожен елемент масиву
	{
		writeStructToConsole(mas[i]);
		cout << endl;
	}
}
void readMasFromFile(ifstream& file, Book*& mas, int& n)         // зчитуємо масив з файлу
{
	file >> n;
	mas = new Book[n];
